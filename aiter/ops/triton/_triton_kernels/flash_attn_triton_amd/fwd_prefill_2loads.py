...
def _attn_fwd_inner(
    ...
    for start_n in range(block_min, block_max, BLOCK_N):
        ...
        # -- compute qk ----
        qk_dot = tl.dot(q1, k1) + tl.dot(q2, k2)
        if IS_FP8:
...
def attn_fwd(
    ...
    offs_d_qk_2 = tl.arange(0, BLOCK_DMODEL_QK_2)
    offs_d_v_1 = tl.arange(0, BLOCK_DMODEL_V_1)
    offs_d_v_2 = tl.arange(0, BLOCK_DMODEL_V_2)
    ...
    q_ptrs_2 = q_offset + offs_m[:, None] * stride_qm + (offs_d_qk_2[None, :] + BLOCK_DMODEL_QK_1) * stride_qk
    ...
    k_ptrs_2 = k_offset + (offs_d_qk_2[:, None] + BLOCK_DMODEL_QK_1) * stride_kk + offs_n[None, :] * stride_kn
    ...
    v_ptrs_2 = v_offset + offs_n[:, None] * stride_vk + (offs_d_v_2[None, :] + BLOCK_DMODEL_V_1) * stride_vn
    ...
    acc2 = tl.zeros([BLOCK_M, BLOCK_DMODEL_V_2], dtype=ACCUMULATOR_TYPE)

    q_mask_m = offs_m[:, None] < seqlen_q
    mask_1 = q_mask_m & (offs_d_qk_1[None, :] < ACTUAL_BLOCK_DMODEL_QK) if PADDED_HEAD_QK_1 else q_mask_m
    mask_2 = q_mask_m & (offs_d_qk_2[None, :] + BLOCK_DMODEL_QK_1 < ACTUAL_BLOCK_DMODEL_QK) if PADDED_HEAD_QK_2 else q_mask_m

    q1 = tl.load(q_ptrs_1, mask=mask_1, other=0.0)
    q2 = tl.load(q_ptrs_2, mask=mask_2, other=0.0)
...
def attention_forward_prefill_triton_impl(
    ...
    def get_split_dims(d):
        d = max(16, d)
        if d <= 32: return 16, 16
        elif d <= 64: return 32, max(16, (d - 32 + 15) // 16 * 16)
        elif d <= 128: return 64, max(16, (d - 64 + 15) // 16 * 16)
        else: return 128, max(16, (d - 128 + 15) // 16 * 16)

    qk_dim1, qk_dim2 = get_split_dims(head_size_qk)
    v_dim1, v_dim2 = get_split_dims(head_size_v)
