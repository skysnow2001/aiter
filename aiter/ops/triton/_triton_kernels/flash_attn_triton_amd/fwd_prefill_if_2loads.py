...
def _attn_fwd_inner(
    ...
    ##
    BLOCK_DMODEL_QK_1 : tl.constexpr,
    # BLOCK_DMODEL_QK_2 : tl.constexpr,
    BLOCK_DMODEL_V_1 : tl.constexpr,
    # BLOCK_DMODEL_V_2 : tl.constexpr,
    ##
    ...
    PADDED_HEAD_QK_1 : tl.constexpr,
    PADDED_HEAD_QK_2 : tl.constexpr,
    PADDED_HEAD_V_1 : tl.constexpr,
    PADDED_HEAD_V_2 : tl.constexpr,
    SPLIT_QK : tl.constexpr,
    ...
    for start_n in range(block_min, block_max, BLOCK_N):
        ...
        k_ptrs_1 = k_base_ptrs_1 + start_n * stride_kn
        if SPLIT_QK:
            k_ptrs_2 = k_base_ptrs_2 + start_n * stride_kn
        v_ptrs_1 = v_base_ptrs_1 + start_n * stride_vk
        if SPLIT_V:
            v_ptrs_2 = v_base_ptrs_2 + start_n * stride_vk

        kv_offs_n = start_n + tl.arange(0, BLOCK_N)

        if APPLY_MASK:
            k_mask_n = kv_offs_n[None, :] < seqlen_k
            v_mask_n = kv_offs_n[:, None] < seqlen_k

            k1_mask = k_mask_n & (offs_d_qk_1[:, None] < ACTUAL_BLOCK_DMODEL_QK) if PADDED_HEAD_QK_1 else k_mask_n
            k1 = tl.load(k_ptrs_1, mask=k1_mask, other=0.0)

            if SPLIT_QK:
                k2_mask = k_mask_n & (offs_d_qk_2[:, None] + BLOCK_DMODEL_QK_1 < ACTUAL_BLOCK_DMODEL_QK) if PADDED_HEAD_QK_2 else k_mask_n
                k2 = tl.load(k_ptrs_2, mask=k2_mask, other=0.0)

            if PRE_LOAD_V:
                v1_mask = v_mask_n & (offs_d_v_1[None, :] < ACTUAL_BLOCK_DMODEL_V) if PADDED_HEAD_V_1 else v_mask_n                
                v1 = tl.load(v_ptrs_1, mask=v1_mask, other=0.0)

                if SPLIT_V:
                    v2_mask = v_mask_n & (offs_d_v_2[None, :] + BLOCK_DMODEL_V_1 < ACTUAL_BLOCK_DMODEL_V) if PADDED_HEAD_V_2 else v_mask_n
                    v2 = tl.load(v_ptrs_2, mask=v2_mask, other=0.0)
        else:
            if PADDED_HEAD_QK_1:
                k1_mask = offs_d_qk_1[:, None] < ACTUAL_BLOCK_DMODEL_QK
                k1 = tl.load(k_ptrs_1, mask=k1_mask, other=0.0)
            else:
                k1 = tl.load(k_ptrs_1)
            if SPLIT_QK:
                if PADDED_HEAD_QK_2:
                    k2_mask = offs_d_qk_2[:, None] + BLOCK_DMODEL_QK_1 < ACTUAL_BLOCK_DMODEL_QK
                    k2 = tl.load(k_ptrs_2, mask=k2_mask, other=0.0)
                else:
                    k2 = tl.load(k_ptrs_2)
            ...
            if PRE_LOAD_V:
                if PADDED_HEAD_V_1:
                    v1_mask = offs_d_v_1[None, :] < ACTUAL_BLOCK_DMODEL_V
                    v1 = tl.load(v_ptrs_1, mask=v1_mask, other=0.0)
                else:
                    v1 = tl.load(v_ptrs_1)
                if SPLIT_V:
                    if PADDED_HEAD_V_2:
                        v2_mask = offs_d_v_2[None, :] + BLOCK_DMODEL_V_1 < ACTUAL_BLOCK_DMODEL_V
                        v2 = tl.load(v_ptrs_2, mask=v2_mask, other=0.0)
                    else:
                        v2 = tl.load(v_ptrs_2)
    ...
    # return acc, l_i, m_i
        ...
        p_casted = p.to(v1.type.element_ty)
        if IS_FP8:
            if FP8_P_DESCALE:
                scale_p, descale_p = compute_fp8_scaling_factors(p, FP8_MAX)
                p_scaled = (p * scale_p).to(v1.type.element_ty)
                acc1 += tl.dot(p_scaled, v1) * descale_p * v_descale
                if SPLIT_V:
                    acc2 += tl.dot(p_scaled, v2) * descale_p * v_descale
            else:
                acc1 += tl.dot(p_casted, v1) * v_descale
                if SPLIT_V:
                    acc2 += tl.dot(p_casted, v2) * v_descale
        else:
            acc1 += tl.dot(p_casted, v1)
            if SPLIT_V:
                acc2 += tl.dot(p_casted, v2)

    return acc1, acc2 if SPLIT_V else acc1, l_i, m_i
...
def attn_fwd(
    ...
    PADDED_HEAD_QK_1: tl.constexpr = ACTUAL_BLOCK_DMODEL_QK < BLOCK_DMODEL_QK_1
    PADDED_HEAD_QK_2: tl.constexpr = ACTUAL_BLOCK_DMODEL_QK < (BLOCK_DMODEL_QK_1 + BLOCK_DMODEL_QK_2)
    PADDED_HEAD_V_1: tl.constexpr = ACTUAL_BLOCK_DMODEL_V < BLOCK_DMODEL_V_1
    PADDED_HEAD_V_2: tl.constexpr = ACTUAL_BLOCK_DMODEL_V < (BLOCK_DMODEL_V_1 + BLOCK_DMODEL_V_2)

    offs_m = start_m * BLOCK_M + tl.arange(0, BLOCK_M)
    offs_n = tl.arange(0, BLOCK_N)
    offs_d_qk_1 = tl.arange(0, BLOCK_DMODEL_QK_1)
    offs_d_qk_2 = tl.arange(0, BLOCK_DMODEL_QK_2) if SPLIT_QK else offs_d_qk_1
    offs_d_v_1 = tl.arange(0, BLOCK_DMODEL_V_1)
    offs_d_v_2 = tl.arange(0, BLOCK_DMODEL_V_2) if SPLIT_V else offs_d_qk_1
    ...
    if total_visible_blocks == 0:
        ...
        o_ptrs_1 = o_offset + offs_m[:, None] * stride_om + offs_d_v_1[None, :] * stride_on
        if SPLIT_V:
            o_ptrs_2 = o_offset + offs_m[:, None] * stride_om + (offs_d_v_2[None, :] + BLOCK_DMODEL_V_1) * stride_on
        
        o_mask_m = offs_m[:, None] < seqlen_q
        o_mask_1 = o_mask_m & (offs_d_v_1[None, :] < ACTUAL_BLOCK_DMODEL_V) if PADDED_HEAD_V_1 else o_mask_m
        tl.store(o_ptrs_1, tl.zeros([BLOCK_M, BLOCK_DMODEL_V_1], dtype=Out.type.element_ty), mask=o_mask_1)
        
        if SPLIT_V:
            o_mask_2 = o_mask_m & (offs_d_v_2[None, :] + BLOCK_DMODEL_V_1 < ACTUAL_BLOCK_DMODEL_V) if PADDED_HEAD_V_2 else o_mask_m
            tl.store(o_ptrs_2, tl.zeros([BLOCK_M, BLOCK_DMODEL_V_2], dtype=Out.type.element_ty), mask=o_mask_2)
    ...
    q_ptrs_1 = q_offset + offs_m[:, None] * stride_qm + offs_d_qk_1[None, :] * stride_qk
    q_ptrs_2 = q_offset + offs_m[:, None] * stride_qm + (offs_d_qk_2[None, :] + BLOCK_DMODEL_QK_1) * stride_qk if SPLIT_QK else q_ptrs_1

    k_offset = K + off_z * stride_kz + off_h_k * stride_kh + cu_seqlens_k_start * stride_kn
    k_ptrs_1 = k_offset + offs_d_qk_1[:, None] * stride_kk + offs_n[None, :] * stride_kn
    k_ptrs_2 = k_offset + (offs_d_qk_2[:, None] + BLOCK_DMODEL_QK_1) * stride_kk + offs_n[None, :] * stride_kn if SPLIT_QK else k_ptrs_1

    v_offset = V + off_z * stride_vz + off_h_k * stride_vh + cu_seqlens_k_start * stride_vk
    v_ptrs_1 = v_offset + offs_n[:, None] * stride_vk + offs_d_v_1[None, :] * stride_vn
    v_ptrs_2 = v_offset + offs_n[:, None] * stride_vk + (offs_d_v_2[None, :] + BLOCK_DMODEL_V_1) * stride_vn if SPLIT_V else v_ptrs_1
    ...
    acc1 = tl.zeros([BLOCK_M, BLOCK_DMODEL_V_1], dtype=ACCUMULATOR_TYPE)
    acc2 = tl.zeros([BLOCK_M, BLOCK_DMODEL_V_2], dtype=ACCUMULATOR_TYPE) if SPLIT_V else acc1

    q_mask_m = offs_m[:, None] < seqlen_q
    mask_1 = q_mask_m & (offs_d_qk_1[None, :] < ACTUAL_BLOCK_DMODEL_QK) if PADDED_HEAD_QK_1 else q_mask_m
    mask_2 = (q_mask_m & (offs_d_qk_2[None, :] + BLOCK_DMODEL_QK_1 < ACTUAL_BLOCK_DMODEL_QK) if PADDED_HEAD_QK_2 else q_mask_m) if SPLIT_QK else mask_1
    

    q1 = tl.load(q_ptrs_1, mask=mask_1, other=0.0)
    q2 = tl.load(q_ptrs_2, mask=mask_2, other=0.0) if SPLIT_QK else q1
    ...
    if n_front_masked_blocks > 0 and USE_SLIDING_WINDOW:
        ...
        acc1, acc2, l_i, m_i = _attn_fwd_inner(
            acc1,               # <--- WAS acc
            acc2,               # <--- NEW
            l_i,
            m_i,
            ...
            offs_m,
            offs_n,
            offs_d_qk_1,        # <--- WAS offs_d_qk
            offs_d_v_1,         # <--- WAS offs_d_v
            offs_d_qk_2,        # <--- NEW
            offs_d_v_2,         # <--- NEW
            block_min, 
            ...
            ##
            BLOCK_DMODEL_QK_1=BLOCK_DMODEL_QK_1,
            # BLOCK_DMODEL_QK_2=BLOCK_DMODEL_QK_2,
            BLOCK_DMODEL_V_1=BLOCK_DMODEL_V_1,
            # BLOCK_DMODEL_V_2=BLOCK_DMODEL_V_2,
            ##
            BLOCK_N=BLOCK_N,
            ...
            ##
            PADDED_HEAD_QK_1=PADDED_HEAD_QK_1,
            PADDED_HEAD_QK_2=PADDED_HEAD_QK_2,
            PADDED_HEAD_V_1=PADDED_HEAD_V_1,
            PADDED_HEAD_V_2=PADDED_HEAD_V_2,
            SPLIT_QK=SPLIT_QK,
            SPLIT_V=SPLIT_V,
            ##
    ...
    acc1 = acc1 * l_recip
    acc2 = acc2 * l_recip if SPLIT_V else acc1
    if ENABLE_DROPOUT:
        dropout_scale = 1 / (1 - dropout_p)
        acc1 = acc1 * dropout_scale
        acc2 = acc2 * dropout_scale if SPLIT_V else acc1
    ...
    if SPLIT_V:
        o_ptrs_2 = o_offset + offs_m[:, None] * stride_om + (offs_d_v_2[None, :] + BLOCK_DMODEL_V_1) * stride_on
        o_mask_2 = tl.full([BLOCK_M, BLOCK_DMODEL_V_2], 1, dtype=tl.int1)

    o_mask_1 = tl.full([BLOCK_M, BLOCK_DMODEL_V_1], 1, dtype=tl.int1)
    

    # 1. Apply sequence length mask if needed
    if overflow_size > 0:
        seq_mask = offs_m[:, None] < seqlen_q
        o_mask_1 = o_mask_1 & seq_mask
        if SPLIT_V:
            o_mask_2 = o_mask_2 & seq_mask

    # 2. Apply head dimension padding mask if needed
    if PADDED_HEAD_V_1:
        o_mask_1 = o_mask_1 & (offs_d_v_1[None, :] < ACTUAL_BLOCK_DMODEL_V)
    # Store chunks
    tl.store(o_ptrs_1, acc1.to(Out.dtype.element_ty), mask=o_mask_1)

    if SPLIT_V:
        if PADDED_HEAD_V_2:
            o_mask_2 = o_mask_2 & (offs_d_v_2[None, :] + BLOCK_DMODEL_V_1 < ACTUAL_BLOCK_DMODEL_V)
        tl.store(o_ptrs_2, acc2.to(Out.dtype.element_ty), mask=o_mask_2)
...
def attention_forward_prefill_triton_impl(
    ...
    def get_split_dims(d):
        d = max(16, d)
        # If perfect power of 2, use single chunk (False flag) and a dummy size for dim2
        if (d & (d - 1) == 0): 
            return False, d, 0
        
        # Otherwise, split into largest power of 2 + remainder
        if d <= 32: return True, 16, max(16, (d - 16 + 15) // 16 * 16)
        elif d <= 64: return True, 32, max(16, (d - 32 + 15) // 16 * 16)
        elif d <= 128: return True, 64, max(16, (d - 64 + 15) // 16 * 16)
        else: return True, 128, max(16, (d - 128 + 15) // 16 * 16)

    split_qk, qk_dim1, qk_dim2 = get_split_dims(head_size_qk)
    split_v, v_dim1, v_dim2 = get_split_dims(head_size_v)
    ...
    attn_fwd[grid](
        ...
        ###
        BLOCK_DMODEL_QK_1=qk_dim1,
        BLOCK_DMODEL_QK_2=qk_dim2,
        BLOCK_DMODEL_V_1=v_dim1,
        BLOCK_DMODEL_V_2=v_dim2,
        SPLIT_QK=split_qk,
        SPLIT_V=split_v,
        ###
